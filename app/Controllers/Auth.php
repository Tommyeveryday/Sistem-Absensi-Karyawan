    ...
    public function register()
    {
        ...
        if ($this->request->getMethod() == 'post') {
            ...
            if (empty($name) || empty($email) || empty($password) || empty($cpassword)) {
                ...
                $register = $this->auth_model->save($data);
                if ($register) {
                    $session->setFlashdata('success', "Your Account has been added successfully.");
                    return redirect()->to('Auth/index'); 
                } else {
                    $session->setFlashdata('error', "Your Account has failed to add.");
                }
            }
        }
        $this->data['session']=$session;
